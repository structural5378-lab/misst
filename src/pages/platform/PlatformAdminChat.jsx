...
export default function PlatformAdminChat() {
  ...
  const { data: messages = [], isLoading, isError, error, refetch } = useQuery({
    ...
    queryFn: async () => (await base44.functions.invoke("adminEntityAdmin", { action: "list", entity: "ChatMessage", community_id: communityFilter || undefined }))?.data?.rows || [],
  });
  ...
  const del = async (id) => {
    try {
      const res = await base44.functions.invoke("adminEntityAdmin", { action: "delete", entity: "ChatMessage", id });
  ...
  const bulkDel = async (rows) => {
    try {
      const res = await base44.functions.invoke("adminEntityAdmin", { action: "bulk_delete", entity: "ChatMessage", ids: rows.map((r) => r.id) });
      if (!res.data?.success) throw new Error(res.data?.error);
