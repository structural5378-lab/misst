...
import { MessageSquare, Eye, Pin, Lock, Star, Megaphone } from "lucide-react";
import { getCategoryMeta, timeAgo, parseJSON, getRoleBadge } from "@/lib/forumUtils";
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 flex-wrap">
          {thread.is_announcement && <MegaphoneBadge />}
          {thread.is_pinned && <PinBadge />}
          {thread.is_featured && <FeaturedBadge />}
          {thread.is_locked && <LockBadge />}
        ...
        <div className="flex items-center gap-2 mt-1.5">
          ...
          {thread.author_callsign && <span className="text-[10px] text-muted-foreground">{thread.author_callsign}</span>}
          <span className="text-[10px] text-muted-foreground/50">·</span>
          <span className="text-[10px] text-muted-foreground">{timeAgo(thread.last_reply_date || thread.created_date)}</span>
        </div>
      </div>
...
function MegaphoneBadge() { return <Megaphone className="w-3.5 h-3.5 text-rose-400 shrink-0" />; }
function PinBadge() { return <Pin className="w-3.5 h-3.5 text-amber-400 shrink-0" />; }
function FeaturedBadge() { return <Star className="w-3.5 h-3.5 text-yellow-400 shrink-0 fill-yellow-400/30" />; }
function LockBadge() { return <Lock className="w-3.5 h-3.5 text-muted-foreground shrink-0" />; }