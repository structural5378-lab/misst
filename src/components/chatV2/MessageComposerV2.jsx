    ...
    <div className="shrink-0 border-t border-border bg-background/80 backdrop-blur-xl">
      ...
      <div className="px-2.5 py-2 mist-safe-bottom flex items-end gap-1.5">
        ...
        <div className="flex items-center gap-0.5 shrink-0 mb-1">
          ...
          <button onClick={() => fileInputRef.current?.click()} className="p-2 rounded-full text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-colors" aria-label="Attach file" title="Attach file"><Paperclip className="w-5 h-5" /></button>
        </div>
        ...
        <button
          ...
          className={`relative shrink-0 w-11 h-11 mb-1 rounded-full flex items-center justify-center transition-all duration-200 ${hasContent ? "mist-send bg-primary text-primary-foreground" : "bg-secondary/60 text-muted-foreground hover:bg-secondary"} ${sending ? "send-pop" : ""}`}
        >
