...
// Compact achievement criteria — mirrors src/lib/achievementsData.js
const CRITERIA = [
...
Deno.serve(async (req) => {
  try {
    ...
    if (body.action === 'leaderboard') {
      const all = await base44.asServiceRole.entities.UserStats.list('-xp', 200);
    ...
    // Calculate XP from all known stats
    const allStats = { ...s, ...updated };
    const xp =
      (allStats.net_checkins || 0) * 50 +
      (allStats.net_control_sessions || 0) * 100 +
      (allStats.forum_posts || 0) * 10 +
      (allStats.photos_uploaded || 0) * 15 +
      (allStats.volunteer_hours || 0) * 25 +
      (allStats.events_attended || 0) * 75 +
      (allStats.repeaters_visited || 0) * 30 +
      (allStats.states_operated || 0) * 100 +
      Math.floor((allStats.miles_traveled || 0) / 10);
    updated.xp = xp;
    updated.level = calcLevel(xp);

    // Check achievements
    const existingAchievements = await base44.asServiceRole.entities.UserAchievement.filter({ user_id: user.id });
    const existingIds = new Set(existingAchievements.map((a: any) => a.achievement_id));
    const newlyUnlocked: any[] = [];

    for (const c of CRITERIA) {
      if (existingIds.has(c.id)) continue;
      const checkStats = { ...allStats, ...updated };
      if (checkCriteria(c, checkStats)) {
    ...
    // Achievement score & count
    const allAch = [...existingAchievements, ...newlyUnlocked];
