...
// it grants the purchaser (or gift recipient) a PremiumBadgeOwnership record
// with a numbered edition, gift metadata, and upgrade flag, then increments the
// badge's purchases_count. Idempotent: duplicate sessions for the same
// user+badge are ignored.
  ...
  try {
    ...
    if (event.type === 'checkout.session.completed') {
      ...
      const {
        ...
        gift_to, gift_to_name, gift_message, scheduled_delivery_at, is_anonymous_gift, is_upgrade,
      } = meta;
      ...
      if (badge_id && ownerId) {
        ...
        if (!existing.length) {
          ...
          const editionNumber = (badge?.purchases_count || 0) + 1;
          const isAnon = is_anonymous_gift === '1';
          const ownerName = gift_to ? (gift_to_name || '') : (user_name || '');
          const giftedByName = gift_to && !isAnon ? (user_name || '') : '';
          ...
          await base44.asServiceRole.entities.PremiumBadgeOwnership.create({
            ...
            badge_edition_size: badge?.edition_size || 0,
            ...
            is_anonymous_gift: isAnon,
            scheduled_delivery_at: scheduled_delivery_at || '',
            is_earned: false,
            is_upgrade: is_upgrade === '1',
